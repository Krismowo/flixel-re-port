...
class FlxTilemap extends FlxObject
{
	...
	public function loadMap(MapData : String, TileGraphic : Class<Dynamic>, TileWidth : Int = 0, TileHeight : Int = 0, AutoTile : Int = OFF, StartingIndex : Int = 0, DrawIndex : Int = 1, CollideIndex : Int = 1) : FlxTilemap
	{
		...
		_tileObjects = new Array<Dynamic>();
		var ac : Int;
		while (i < l)
		{
	...
	//Create a distance-based representation of the tilemap.
	{
		...
		while (i < mapSize)
		{
			if (!Math.isNaN((try cast(Reflect.field(_tileObjects, Std.string(_data[i])), FlxTile) catch (e:Dynamic) null).allowCollisions))
					{
						distances[i] = -2;
					}
			else
	...
	public function overlapsWithCallback(Object : FlxObject, Callback : Function = null, FlipCallbackParams : Bool = false, Position : FlxPoint = null) : Bool
	{
		...
		var selectionX : Int = Std.int(FlxU.floor((Object.x - X) / _tileWidth));
		var selectionY : Int = Std.int(FlxU.floor((Object.y - Y) / _tileHeight));
		var selectionWidth : Int = as3hx.Compat.parseInt(selectionX + (FlxU.ceil(Object.width / _tileWidth)) + 1);
		var selectionHeight : Int = as3hx.Compat.parseInt(selectionY + FlxU.ceil(Object.height / _tileHeight) + 1);
		...
		while (row < selectionHeight)
		{
			...
			while (column < selectionWidth)
			{
				...
				if (!Math.isNaN(tile.allowCollisions))
				{
					...
					if (overlapFound)
					{
						if ((tile.callback != null) && ((tile.filter == null) || (Std.is(Object, tile.filter))))
						{
				...
				else if ((tile.callback != null) && ((tile.filter == null) || (Std.is(Object, tile.filter))))
				{
					tile.mapIndex = rowStart + column;
					tile.callback(tile, Object);
				}
	...
	{
		return try cast(_data[Y * widthInTiles + X], Int) catch (e:Dynamic) 0;
	}
	...
	{
		return try cast(_data[Index], Int) catch (e:Dynamic) 0;
	}
	...
	public function ray(Start : FlxPoint, End : FlxPoint, Result : FlxPoint = null, Resolution : Float = 1) : Bool
	{
		...
		while (i < steps)
		{
			...
			if (!Math.isNaN((try cast(Reflect.field(_tileObjects, Std.string(_data[tileY * widthInTiles + tileX])), FlxTile) catch (e:Dynamic) null).allowCollisions))
					{
