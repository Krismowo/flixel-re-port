...
/**
	 ...
	 * @author Adam Atomic
	 */
@:bitmap("org/flixel/data/vcr/open.png")
class ImgOpen extends BitmapData {}
@:bitmap("org/flixel/data/vcr/record_off.png")
class ImgRecordOff extends BitmapData {}
@:bitmap("org/flixel/data/vcr/record_on.png")
class ImgRecordOn extends BitmapData {}
@:bitmap("org/flixel/data/vcr/stop.png")
class ImgStop extends BitmapData {}
@:bitmap("org/flixel/data/vcr/flixel.png")
class ImgFlixel extends BitmapData {}
@:bitmap("org/flixel/data/vcr/restart.png")
class ImgRestart extends BitmapData {}
@:bitmap("org/flixel/data/vcr/pause.png")
class ImgPause extends BitmapData {}
@:bitmap("org/flixel/data/vcr/play.png")
class ImgPlay extends BitmapData {}
@:bitmap("org/flixel/data/vcr/step.png")
class ImgStep extends BitmapData {}
...
class VCR extends Sprite
{
	...
	public function new()
	{
		...
		_open = new Bitmap(Type.createInstance(ImgOpen, []));
		addChild(_open);

		_recordOff = new Bitmap(Type.createInstance(ImgRecordOff, []));
		_recordOff.x = _open.x + _open.width + spacing;
		addChild(_recordOff);

		_recordOn = new Bitmap(Type.createInstance(ImgRecordOn, []));
		_recordOn.x = _recordOff.x;
		_recordOn.visible = false;
		addChild(_recordOn);

		_stop = new Bitmap(Type.createInstance(ImgStop, []));
		_stop.x = _recordOff.x;
		_stop.visible = false;
		addChild(_stop);

		_flixel = new Bitmap(Type.createInstance(ImgFlixel, []));
		_flixel.x = _recordOff.x + _recordOff.width + spacing;
		addChild(_flixel);

		_restart = new Bitmap(Type.createInstance(ImgRestart, []));
		_restart.x = _flixel.x + _flixel.width + spacing;
		addChild(_restart);

		_pause = new Bitmap(Type.createInstance(ImgPause, []));
		_pause.x = _restart.x + _restart.width + spacing;
		addChild(_pause);

		_play = new Bitmap(Type.createInstance(ImgPlay, []));
		_play.x = _pause.x;
		_play.visible = false;
		addChild(_play);

		_step = new Bitmap(Type.createInstance(ImgStep, []));
